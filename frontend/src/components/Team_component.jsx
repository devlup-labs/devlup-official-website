...
const Tile = React.memo(({ tile, TILE_SIZE, openTile, tileRefs, isActive, focusProgress }) => {
  ...
  const handleViewProfile = (e) => { e.stopPropagation(); navigate(`/portfolio/${tile.memberId}`); };
    ...
    <div ref={(el) => (tileRefs.current[tile.id] = el)} className={`absolute top-0 left-0 [will-change:transform] ${isActive ? 'z-[5000]' : 'z-1'}`}>
      <div onClick={(e) => { e.stopPropagation(); if (!tile.isEmpty) openTile(tile.id); }} className={`relative flex flex-col items-center justify-center transition-colors duration-500 ${isActive ? "rounded-full" : "rounded-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] backdrop-blur-md"}`} style={{ width: TILE_SIZE, height: TILE_SIZE, backfaceVisibility: "hidden", border: `3px solid ${tile.isEmpty ? "#E5E7EB" : getBorderColor(tile.designation)}`, boxShadow: tile.isEmpty ? "none" : `0 0 10px ${getBorderColor(tile.designation)}80` }}>
        ...
        <div className={`absolute inset-0 rounded-full overflow-hidden transition-opacity duration-500 ${isActive ? 'opacity-0' : 'opacity-100'}`}>
          {!imageLoaded && (<div className="w-full h-full flex items-center justify-center bg-gray-700"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-white"></div></div>)}
          <img src={tile.isEmpty ? "https://static.vecteezy.com/system/resources/previews/036/280/651/non_2x/default-avatar-profile-icon-social-media-user-image-gray-avatar-icon-blank-profile-silhouette-illustration-vector.jpg" : tile.profileImage} alt={tile.name || "default"} className="w-full h-full object-cover" onLoad={() => setImageLoaded(true)} onError={() => setImageLoaded(true)} />
        </div>

        {isActive && !tile.isEmpty && (
          <div className="absolute flex flex-col items-center justify-center rounded-full shadow-2xl border backdrop-blur-xl" style={{ width: window.innerWidth < 768 ? '320px' : '420px', height: window.innerWidth < 768 ? '320px' : '420px', transform: `scale(${focusProgress})`, opacity: focusProgress, border: `3px solid ${tile.isEmpty ? "#E5E7EB" : getBorderColor(tile.designation)}` }}>
            {showProfileUI && (
              <div className="flex flex-col items-center w-full px-8 animate-in fade-in zoom-in duration-300">
                <div className="flex flex-row items-center gap-4 mb-4 w-full">
                  <div className="w-24 h-24 md:w-32 md:h-32 rounded-full border-2 border-white/40 overflow-hidden shadow-lg shrink-0 self-start">
                    {!profileImageLoaded && (<div className="w-full h-full flex items-center justify-center bg-gray-700"><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div></div>)}
                    <img src={tile.profileImage} alt={tile.name} className="w-full h-full object-cover" onLoad={() => setProfileImageLoaded(true)} onError={() => setProfileImageLoaded(true)} />
                  </div>
                  <div className="flex flex-col pt-1">
                    ...
                    <div className="flex gap-3 mt-2 text-lg"><a href={tile.github} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}><FontAwesomeIcon icon={faGithub} /></a><a href={`mailto:${tile.email}`} onClick={(e) => e.stopPropagation()}><FontAwesomeIcon icon={faEnvelope} /></a><a href={tile.linkedin} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}><FontAwesomeIcon icon={faLinkedin} /></a></div>
                  </div>
                </div>
                <div className="border-t border-black/10 pt-3 w-full text-center"><span className="text-[10px] font-bold opacity-40 uppercase tracking-widest">{tile.tag}</span><p className="text-[11px] leading-tight mt-1 line-clamp-3 px-4">{tile.bio}</p></div>
                <button onClick={handleViewProfile} className="mt-4 px-6 py-2 bg-[var(--bg-muted)] text-white text-[10px] font-bold rounded-full hover:bg-[var(--bg-muted)] transition-all">VIEW PROFILE</button>
              </div>
            )}
          </div>
        )}
...
function TeamComponent() {
  ...
  useEffect(() => { if (hamburgerOpen && (searchOpen || filterOpen)) { setSearchOpen(false); setFilterOpen(false); } }, [hamburgerOpen]);

  useEffect(() => { if ((searchOpen || filterOpen) && hamburgerOpen) setHamburgerOpen(false); }, [searchOpen, filterOpen]);
  ...
  const MAX_SCROLL_ROTATION = 360;

  useEffect(() => { getTeam().then(res => { const formatted = res.data.data.map(item => ({ memberId: item.member_id, name: item.member_name, profileImage: item.member_image, rollNumber: item.member_roll_number, designation: item.member_designation, tag: item.member_tag, bio: item.member_about, github: item.member_github_id, linkedin: item.member_linkedin, email: item.member_email })); setMembers(formatted); }).catch(err => console.error("API error:", err)); }, []);
  ...
  const triggerOpen = (id) => { setActiveTile(id); animateToCenter(id); };
  ...
  const handleSearchKeyDown = (e) => {
    if (e.key === "Enter" && searchTerm.trim() !== "") {
      ...
      if (match) snapToMember(match.id);
    }
  };

  useEffect(() => {
    const applyRotation = () => { rotationRef.current = scrollBaseRotationRef.current + horizontalOffsetRef.current; setRotation(rotationRef.current); };

    const onScroll = () => {
      if (activeTile) return;
      const sectionEl = sectionRef.current; if (!sectionEl) return;
      const totalScrollable = sectionEl.offsetHeight - window.innerHeight; if (totalScrollable <= 0) return;
    ...
    const onTouchStart = (e) => { if (!sectionRef.current) return; const rect = sectionRef.current.getBoundingClientRect(); const inView = rect.bottom > 0 && rect.top < window.innerHeight; if (!inView) return; isTouchingRef.current = true; touchStartRef.current = e.touches[0].clientX; };
    const onTouchMove = (e) => { if (activeTile || !isTouchingRef.current) return; const currentX = e.touches[0].clientX; const delta = (touchStartRef.current - currentX) * HORIZONTAL_TOUCH_FACTOR; touchStartRef.current = currentX; horizontalOffsetRef.current -= delta; applyRotation(); };
  ...
  const filteredMembers = useMemo(() => members.filter((m) => {
    ...
    if (typeof tags === "string") tags = [tags.trim().toLowerCase()];
    return selectedTags.length === 0 || tags.some((tag) => selectedTags.includes(tag));
  }), [members, selectedTags]);

  const allTags = useMemo(() => {
    const tagSet = new Set();
    members.forEach((m) => { if (m.designation) tagSet.add(m.designation.trim().toLowerCase()); else if (m.tag) tagSet.add(m.tag.trim().toLowerCase()); });
  ...
  const tilesData = useMemo(() => {
    const out = [];
    const angleStep = 360 / COLS;
    const FRONT_COLS = Math.min(Math.floor(COLS / 2), Math.max(4, Math.ceil(filteredMembers.length * 1.5)));
    const TOTAL_VISIBLE = ROWS * FRONT_COLS;
    const startIndex = Math.floor((TOTAL_VISIBLE - filteredMembers.length) / 2);
    let visibleCounter = 0;
    for (let row = 0; row < ROWS; row++) {
      for (let i = 0; i < COLS; i++) {
        const isFront = i < FRONT_COLS;
        let member = null;
        if (isFront) {
          if (visibleCounter >= startIndex && visibleCounter < startIndex + filteredMembers.length) member = filteredMembers[visibleCounter - startIndex];
          visibleCounter++;
        }
        out.push({ id: `${row}-${i}`, ...(member || {}), isEmpty: !member, baseAngle: i * angleStep + (row % 2 ? angleStep / 2 : 0), row });
      }
    }
    return out;
  }, [filteredMembers]);
  ...
  useEffect(() => {
    ...
    if (activeTile) { const [r, idx] = activeTile.split("-").map(Number); activeAngle = idx * angleStep + (r % 2 ? angleStep / 2 : 0) + rotation; }
    ...
    tilesData.forEach((tile) => {
      ...
      let splitX = 0;
      if (activeAngle !== null && tile.id !== activeTile) { const d = ((angle - activeAngle + 540) % 360) - 180; splitX = (d < 0 ? -SPLIT_SHIFT : SPLIT_SHIFT) * focusProgress; }
      ...
      if (isActive) { exX = -x * focusProgress; exY = -curvedY * focusProgress; exZ = (isMobile ? 280 : 600 - z) * focusProgress; }
      ...
      <div className="fixed left-0 right-0 z-[1001] flex gap-3 items-center justify-center w-full pointer-events-none top-0 h-[88px] bg-transparent">
        <div className="flex items-center relative pointer-events-auto" onMouseDown={(e) => e.stopPropagation()}>
          <div className={`flex items-center bg-[var(--bg-muted)] border border-[var(--border-subtle)] rounded-full transition-all duration-500 ease-out pointer-events-auto ${searchOpen ? (isMobile ? "w-[calc(100vw-80px)] h-10 px-4 justify-start" : "w-[300px] h-10 px-4 justify-start") : "w-10 h-10 justify-center"}`} onMouseDown={(e) => e.stopPropagation()}>
            ...
            <input ref={searchInputRef} type="text" placeholder="Search team..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={handleSearchKeyDown} onMouseDown={(e) => e.stopPropagation()} className={`bg-transparent border-none outline-none text-white text-sm transition-all duration-300 pointer-events-auto ${searchOpen ? "w-full ml-3 opacity-100" : "w-0 opacity-0"}`} autoComplete="off" />
          </div>
        </div>
        ...
        <div className="pointer-events-auto" onMouseDown={(e) => e.stopPropagation()}>
          <div className={`flex items-center overflow-visible bg-[var(--bg-muted)] backdrop-blur-md border border-white/10 transition-all ease-in-out ${hamburgerOpen ? "duration-0" : "duration-500"} pointer-events-auto ${filterOpen ? "pr-3" : ""} rounded-full`} onMouseDown={(e) => e.stopPropagation()}>
            <button onClick={(e) => { e.stopPropagation(); const newState = !filterOpen; setFilterOpen(newState); if (newState && isMobile) setSearchOpen(false); }} className={`flex items-center justify-center text-white w-10 h-10 md:w-auto md:px-4 transition-all duration-300 cursor-pointer pointer-events-auto`}>
              <FaTags size={16} />
              <span className={`ml-2 text-xs text-white font-medium hidden md:block transition-opacity duration-300 ${filterOpen ? "opacity-100" : "opacity-100"}`}>Filters</span>
            </button>
