          ...
          <div className="flex items-center ml-auto">
            <div className={`hidden md:flex items-center gap-1 ${hamburgerOpen ? 'px-3 py-2 bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl' : 'px-0 py-0 bg-transparent border-transparent rounded-xl'} transition-transform duration-200 ${!hamburgerOpen ? "hover:scale-110" : ""}`}>
              <nav className={`flex items-center gap-0 overflow-hidden transition-all duration-300 ease-out ${hamburgerOpen ? "max-w-[600px] opacity-100" : "max-w-0 opacity-0"}`}>
                {navItems.map((item) => (<Link key={item.name} to={item.path} onClick={() => setHamburgerOpen(false)} className="flex h-10 items-center rounded-lg px-6 text-[var(--text-primary)] whitespace-nowrap transition hover:bg-black/5 dark:hover:bg-white/5">{item.name}</Link>))}
            ...
            <button type="button" onClick={handleHamburgerClick} aria-label="Toggle navigation menu" className="hidden md:flex relative ml-2 h-14 w-14 items-center justify-center rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] transition hover:scale-110 cursor-pointer">
              <span className="pointer-events-none flex flex-col gap-1.5"><span className={`block h-0.5 w-6 bg-[var(--text-primary)] transition-all ${hamburgerOpen ? "rotate-45 translate-y-2" : ""}`} /><span className={`block h-0.5 w-6 bg-[var(--text-primary)] transition-all ${hamburgerOpen ? "opacity-0" : ""}`} /><span className={`block h-0.5 w-6 bg-[var(--text-primary)] transition-all ${hamburgerOpen ? "-rotate-45 -translate-y-2" : ""}`} /></span>
            </button>

            <button type="button" onClick={handleHamburgerClick} aria-label="Toggle navigation menu" className="md:hidden flex h-14 w-14 items-center justify-center rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] z-50 transition hover:scale-110 cursor-pointer"><span className="pointer-events-none flex flex-col gap-1.5"><span className={`block h-0.5 w-6 bg-[var(--text-primary)] transition-all ${hamburgerOpen ? "rotate-45 translate-y-2" : ""}`} /><span className={`block h-0.5 w-6 bg-[var(--text-primary)] transition-all ${hamburgerOpen ? "opacity-0" : ""}`} /><span className={`block h-0.5 w-6 bg-[var(--text-primary)] transition-all ${hamburgerOpen ? "-rotate-45 -translate-y-2" : ""}`} /></span></button>
