...
export default function PodcastComponent() {
  ...
  useEffect(() => {
    ...
    getPodcasts()
      .then((res) => {
        ...
        if (!podcastData) podcastData = res.data;
  ...
  const updateTransforms = () => {
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      let clickScale = 1;
      if (currentIndex === i) clickScale = 1.05;
      else if (currentIndex !== null) clickScale = 0.96;
      ...
      el.style.opacity = distance > 6 ? 0 : currentIndex !== null ? (currentIndex === i ? 1 : 0.4) : 1;

      const brightness = currentIndex !== null && currentIndex !== i ? 0.4 : 1;

      el.style.filter = currentIndex !== null && currentIndex !== i ? "blur(4px) brightness(0.4)" : `brightness(${brightness})`;

      el.style.zIndex = currentIndex === i ? 5000 : Math.round(1000 - distance * 50);
    }
  };
  ...
  const animate = () => {
    ...
    if (targetIndex.current !== null || Math.abs(velocity.current) > 0.0005 || Math.abs(scrollPos.current - newIndex) > 0.0005) {
                ...
                <div className="p-4">
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center justify-center gap-3 flex-wrap">
                      ...
                      <button onClick={() => skipTime(-5)} className="w-12 h-12 rounded-xl flex items-center justify-center bg-gradient-to-br from-blue-400 to-blue-600 shadow-md text-white text-xs font-semibold">-5s</button>

                      <button onClick={() => setIsPlaying(!isPlaying)} className="w-16 h-16 rounded-full flex items-center justify-center bg-gradient-to-br from-blue-400 to-blue-600 shadow-[0_12px_40px_rgba(0,0,0,0.35)]">
                        {isPlaying ? (
                          <div className="flex gap-[5px]"><div className="w-[5px] h-6 bg-white rounded-sm" /><div className="w-[5px] h-6 bg-white rounded-sm" /></div>
                        ) : (
                      ...
                      <div className="relative">
                        <button onClick={() => setShowSpeedMenu((prev) => !prev)} className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-400 to-blue-600 text-white text-sm">{speed}x</button>
                        ...
                        {showSpeedMenu && (
                          <div className="absolute right-0 top-full mt-2 bg-gradient-to-br from-blue-400 to-blue-600 rounded-xl py-2 px-2 flex flex-col gap-1 text-sm text-white z-50 shadow-xl">
                            {speeds.map((s) => (
                              <button key={s} onClick={() => { setSpeed(s); setShowSpeedMenu(false); }} className={`px-3 py-1 rounded-md transition ${speed === s ? "bg-white/20" : "hover:bg-white/10"}`}>
                                {s}x
                              </button>
                            ))}
                          </div>
                    ...
                    <div className="w-full flex items-center gap-3">
                      ...
                      <input type="range" min="0" max={duration || 0} value={progress} onChange={handleSeek} className="w-full accent-blue-500" />
              ...
              <div className="relative w-full h-full flex items-center justify-center">
                    ...
                    {items.map((item, index) => (
                      <div key={item.id} onClick={() => {
                        ...
                        <div className="w-full h-full rounded-2xl overflow-hidden transition-transform duration-300 hover:scale-105">
                          {!imageLoaded[item.id] && (
                            <div className="flex h-full w-full items-center justify-center bg-gray-700"><div className="h-6 w-6 animate-spin rounded-full border-b-2 border-white"></div></div>
                          )}
                          <img src={item.img} className="h-full w-full object-cover" onLoad={() => setImageLoaded(prev => ({ ...prev, [item.id]: true }))} onError={() => setImageLoaded(prev => ({ ...prev, [item.id]: true }))} />
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="absolute right-[2%] lg:right-[5%] xl:right-[10%] top-1/2 -translate-y-1/2 w-[460px]">
                  <div className="flex flex-col gap-6 h-full">
                    ...
                    {active && (
                      <>
                        <div className="flex justify-between items-center"><span className="text-sm opacity-60">{active.date}</span><span className="text-sm opacity-60">{active.author}</span></div>

                        <h1 className="text-3xl font-bold uppercase">{active.title}: {active.subtitle}</h1>
                    ...
                    <div className="flex-1 flex items-end justify-center">
                      ...
                      {clickedIndex === null ? (
                        <button onClick={() => { setClickedIndex(activeIndex); setIsPlaying(true); }} className="w-24 h-24 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center shadow-xl"><div className="w-0 h-0 border-l-[22px] border-l-white border-y-[12px] border-y-transparent ml-[6px]" /></button>
                      ) : (
                        <div className="flex flex-col items-center gap-6">
                          <div className="flex items-center gap-8">
                            <button onClick={() => { setClickedIndex(null); setIsPlaying(false); }} className="w-14 h-14 rounded-2xl flex items-center justify-center bg-gradient-to-br from-blue-400 to-blue-600 shadow-md hover:scale-110 active:scale-95 transition">
                              <svg viewBox="0 0 24 24" className="w-6 h-6 stroke-white" fill="none" strokeWidth="2.5"><path d="M14 3h5v18h-5" strokeLinecap="round" /><path d="M10 12H3" strokeLinecap="round" /><path d="M6 9l-3 3 3 3" strokeLinecap="round" strokeLinejoin="round" /><path d="M10 6v3" strokeLinecap="round" /><path d="M10 15v3" strokeLinecap="round" /></svg>
                            </button>

                            <button onClick={() => skipTime(-5)} className="w-14 h-14 rounded-2xl flex items-center justify-center bg-gradient-to-br from-blue-400 to-blue-600 shadow-md text-white font-semibold text-sm hover:scale-110 active:scale-95 transition">-5s</button>

                            <button onClick={() => setIsPlaying(!isPlaying)} className="w-24 h-24 rounded-full flex items-center justify-center bg-gradient-to-br from-blue-400 to-blue-600 shadow-[0_12px_40px_rgba(0,0,0,0.35)] hover:scale-110 transition">
                              {isPlaying ? (<div className="flex gap-[6px]"><div className="w-[6px] h-7 bg-white rounded-sm" /><div className="w-[6px] h-7 bg-white rounded-sm" /></div>) : (<div className="w-0 h-0 border-l-[22px] border-l-white border-y-[12px] border-y-transparent ml-[6px]" />)}
                            </button>

                            <button onClick={() => skipTime(5)} className="w-14 h-14 rounded-2xl flex items-center justify-center bg-gradient-to-br from-blue-400 to-blue-600 shadow-md text-white font-semibold text-sm hover:scale-110 active:scale-95 transition">+5s</button>

                            <div className="relative">
                              <button onClick={() => setShowSpeedMenu((prev) => !prev)} className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-400 to-blue-600 text-white">{speed}x</button>
                              {showSpeedMenu && (
                                <div className="absolute left-full ml-3 top-1/2 -translate-y-1/2 bg-gradient-to-br from-blue-400 to-blue-600 backdrop-blur-md rounded-xl py-2 px-3 flex flex-col gap-1 text-sm text-white z-50 shadow-xl">
                                  {speeds.map((s) => (
                                    <button key={s} onClick={() => { setSpeed(s); setShowSpeedMenu(false); }} className={`px-3 py-1 rounded-md transition ${speed === s ? "bg-white/20" : "hover:bg-white/10"}`}>{s}x</button>
                                  ))}
                                </div>
                              )}
                            </div>
