    ...
    <div className="flex min-h-screen bg-[#f8fafc]">
      ...
      <div className={`fixed md:sticky top-0 left-0 h-screen z-40 transition-all duration-300 ${
        ...
        <div className="p-6 flex items-center justify-between">
          <h1 className="font-bold text-xl tracking-tight text-blue-400">DEVLUP</h1>
          <button 
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto w-full">
          {activeTab === 'dashboard' ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
             <StatCard title="Home Section" count={counts.home} icon={<Home size={20} md:size={24} color="#1d4ed8" />} color="bg-blue-500 text-blue-500" tabId="home" />
<StatCard title="Team Members" count={counts.team} icon={<Users size={20} md:size={24} color="#16a34a" />} color="bg-green-500 text-green-500" tabId="team" />
<StatCard title="Podcasts" count={counts.podcast} icon={<Mic2 size={20} md:size={24} color="#7c3aed" />} color="bg-purple-500 text-purple-500" tabId="podcast" />
<StatCard title="Videos" count={counts.video} icon={<Video size={20} md:size={24} color="#ea580c" />} color="bg-orange-500 text-orange-500" tabId="video" />
<StatCard title="Blog Posts" count={counts.blog} icon={<FileText size={20} md:size={24} color="#db2777" />} color="bg-pink-500 text-pink-500" tabId="blog" />
<StatCard title="Timeline Events" count={counts.timeline} icon={<Clock size={20} md:size={24} color="#14b8a6" />} color="bg-teal-500 text-teal-500" tabId="timeline" />
            </div>
          ) : (
                ...
                <div className="flex flex-col gap-3 md:flex-row md:justify-between md:items-center mt-4 md:mt-0">
                  ...
                  <div className="relative flex-1 md:flex-none">
                    ...
                    <input
                      ...
                      className="w-full md:w-auto pl-9 pr-3 py-2 border border-slate-300 rounded-lg text-sm"
                    />
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 md:gap-3 w-full md:w-auto">
                    ...
                    {(activeTab === 'video' || activeTab === 'blog' || activeTab === 'podcast') && (
                      <select
                        ...
                        className="border border-slate-300 rounded-lg px-3 py-2 text-sm w-full md:w-auto"
                      >
                        {allTags.map(tag => (
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredItems.map((item) => (
                      <tr key={getItemId(item)} className="hover:bg-slate-50/50 transition-colors">
                        <td className="p-4 font-medium text-slate-700 truncate">{getItemTitle(item)}</td>
                        <td className="p-4 text-slate-500 truncate">{getItemAuthor(item)}</td>
              ...
              <div className="md:hidden divide-y divide-slate-100">
                {filteredItems.map((item) => (
                  <div key={getItemId(item)} className="p-4 hover:bg-slate-50/50 transition-colors">
                    ...
                    <div className="mb-4">
                      <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Author/Role</p>
                      <p className="text-sm text-slate-500 mt-1">{getItemAuthor(item)}</p>
                    </div>
                    <div className="flex gap-2 justify-end">
                      <button onClick={() => handleEdit(item)} className="p-2 text-slate-400 hover:text-blue-700 transition-colors bg-slate-50 rounded-lg"><Edit3 size={18}/></button>
                      <button onClick={() => deleteItem(getItemId(item))} className="p-2 text-slate-400 hover:text-red-800 transition-colors bg-slate-50 rounded-lg"><Trash2 size={18}/></button>
                    </div>
                  </div>
                ))}
              </div>
